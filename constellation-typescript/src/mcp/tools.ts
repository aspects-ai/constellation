...
type WorkspaceGetter = (sessionId?: string) => Workspace
 ...
 */
export function registerTools(server: McpServer, getWorkspace: WorkspaceGetter): void {
  ...
  server.registerTool(
    ...
    async ({ path: filePath }, { sessionId }) => {
      const workspace = getWorkspace(sessionId)
      const buffer = await workspace.readFile(filePath, null) as Buffer
  ...
  server.registerTool(
    ...
    async ({ path: filePath, edits, dryRun }, { sessionId }) => {
      ...
      if (!dryRun && modified !== normalizeLineEndings(original)) {
        await workspace.writeFile(filePath, modified)
      }
  ...
  server.registerTool(
    ...
    async ({ path: dirPath }, { sessionId }) => {
      const workspace = getWorkspace(sessionId)
      await workspace.mkdir(dirPath, { recursive: true })
      return {
    ...
    async ({ path: dirPath }, { sessionId }) => {
      ...
      const formatted = await Promise.all(
        entries.map(async (entry) => {
          try {
            const stats = await workspace.stat(path.join(dirPath, entry))
  ...
  server.registerTool(
    ...
    async ({ path: dirPath, excludePatterns }, { sessionId }) => {
      ...
      async function buildTree(currentPath: string): Promise<TreeNode[]> {
        const entries = await workspace.readdir(currentPath) as string[]
        ...
        for (const entry of entries) {
          ...
          try {
            const stats = await workspace.stat(entryPath)
  ...
  server.registerTool(
    ...
    async ({ source, destination }, { sessionId }) => {
      ...
      if (destDir && destDir !== '.') {
        await workspace.mkdir(destDir, { recursive: true })
      }

      // Use exec for the actual move - path validation is handled by workspace
      // The workspace.exists() calls above already validate the paths are within bounds
      await workspace.exec(`mv "${source}" "${destination}"`)
      return {
    ...
    async ({ path: searchPath, pattern, excludePatterns }, { sessionId }) => {
      ...
      async function searchDir(currentPath: string): Promise<void> {
        ...
        for (const entry of entries) {
          ...
          try {
            const stats = await workspace.stat(fullPath)
            if (stats.isDirectory()) {
  ...
  server.registerTool(
    ...
    async ({ path: filePath }, { sessionId }) => {
      const workspace = getWorkspace(sessionId)
      const stats = await workspace.stat(filePath)
