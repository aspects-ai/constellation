...
import { join, isAbsolute, resolve, relative, normalize } from "path";
import { existsSync, readFileSync, mkdirSync, lstatSync, readlinkSync } from "fs";
...
const BaseBackendConfigSchema = z.object({
  ...
  onDangerousOperation: z.function().args(z.string()).returns(z.void()).optional(),
...
class POSIXCommands {
   ...
   */
  static mkdir(path, parents = true) {
   ...
   */
  static cut(fields, path, delimiter = "	") {
    ...
    for (const segment of segments) {
      ...
      try {
        ...
        if (stats.isSymbolicLink()) {
          ...
          const resolvedLink = isAbsolute(linkTarget) ? linkTarget : resolve(currentPath, "..", linkTarget);
